...
impl ModalManager {
    ...
    pub fn render(
        ...
        if let Some(size) = modal_size {
            // Render dimmer at z-index 20
            elements.push(
                Element::new(&fonts.body, ElementContent::Text(String::new()))
                .colors(ElementColors {
                    border: BorderColor::default(),
                    bg: LinearRgba(0.0, 0.0, 0.0, opacity).into(),
                    text: LinearRgba(1.0, 1.0, 1.0, 1.0).into(),
                })
                .display(DisplayType::Block)
                .min_width(Some(Dimension::Pixels(sidebar_bounds.width() as f32)))
                .min_height(Some(Dimension::Pixels(sidebar_bounds.height() as f32)))
                .zindex(20)
            );

            // Calculate modal bounds based on size
            let modal_bounds = self.calculate_modal_bounds(
                size,
                sidebar_bounds,
                window_bounds,
            );
            ...
            elements.push(
                ...
                .colors(ElementColors {
                    border: BorderColor::default(),
                    bg: LinearRgba(0.1, 0.1, 0.1, opacity).into(),
                    text: LinearRgba(1.0, 1.0, 1.0, 1.0).into(),
                })
                .display(DisplayType::Block)
                .min_width(Some(Dimension::Pixels(modal_bounds.width() as f32)))
            ...
            let close_button_bounds: RectF = euclid::rect(
                modal_bounds.max_x() - close_button_size - 8.0,
            ...
            elements.push(
                Element::new(&fonts.heading, ElementContent::Text("×".to_string()))
                    .colors(ElementColors {
                        border: BorderColor::default(),
                        bg: LinearRgba(0.0, 0.0, 0.0, 0.0).into(),
                        text: LinearRgba(0.7, 0.7, 0.7, opacity).into(),
                    })
    ...
    fn render_scrollbar(&self, modal_bounds: RectF, fonts: &SidebarFonts) -> Vec<Element> {
        ...
        let track_container = Element::new(&fonts.body, ElementContent::Children(vec![
            // Track background
            Element::new(&fonts.body, ElementContent::Text(String::new()))
            .colors(ElementColors {
                border: BorderColor::default(),
    ...
    pub fn handle_mouse_event(&mut self, event: &window::MouseEvent, sidebar_bounds: RectF) -> bool {
        ...
        if let Some(ref modal) = self.active_modal {
            if let Some(modal_bounds) = modal.position {
                ...
                let scrollbar_bounds = if scrollbar_visible {
                    ...
                    let scrollbar_x = modal_bounds.max_x() - scrollbar_width - scrollbar_padding;
                    let scrollbar_y = modal_bounds.min_y() + 40.0;
                    Some(euclid::rect(scrollbar_x, scrollbar_y, scrollbar_width, self.visible_height))
                } else {
                    None
                };
                ...
                match &event.kind {
                    WMEK::Move => {
                        ...
                        if self.dragging_scrollbar {
                            if let (Some(drag_start_y), Some(drag_start_offset)) = (self.drag_start_y, self.drag_start_offset) {
                                let delta_y = event.coords.y as f32 - drag_start_y;
                                let scrollbar_y = modal_bounds.min_y() + 40.0;
                                let thumb_height = (self.visible_height / self.content_height) * self.visible_height;
                                ...
                                if max_thumb_travel > 0.0 {
                                    let thumb_position = (delta_y / max_thumb_travel).clamp(0.0, 1.0);
                                    let max_scroll = (self.content_height - self.visible_height).max(0.0);
