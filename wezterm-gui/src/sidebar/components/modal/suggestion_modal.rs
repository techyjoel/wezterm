...
impl ModalContent for SuggestionModal {
    fn render(&self, context: &ModalRenderContext) -> Element {
        ...
        if self.suggestion.has_action {
            ...
            let button_container = Element::new(
                &context.fonts.body,
                ElementContent::Children(button_row)
            )
        ...
        let button_height = if self.suggestion.has_action { 40.0 } else { 0.0 };
        
        let estimated_height = title_height + (estimated_lines * line_height) + button_height + 40.0;
            ...
            ModalEvent::Mouse(mouse_event) => {
                ...
                match &mouse_event.kind {
                    WMEK::Press(MousePress::Left) => {
                        let point = euclid::point2(mouse_event.coords.x as f32, mouse_event.coords.y as f32);
