    ...
    fn apply_macos_border(&mut self, border: Option<&crate::os::parameters::OsBorderStyle>) {
        ...
        unsafe {
            if let Some(border_style) = border {
                ...
                // Remove/hide border
                if let Some(border_layer) = self.get_border_layer() {
                    let _: () = msg_send![border_layer, setHidden: YES];
                    log::debug!("Removed macOS border");
                }
            }
        }
    }
    ...
    fn cleanup_border_layers(&self) {
        unsafe {
            ...
            for i in 0..count {
                ...
                if !name.is_null() {
                    let name_str = nsstring_to_str(name);
                    if name_str == "WezTermBorderLayer" {
                        layers_to_remove.push(sublayer);
                    }
                }
            }
